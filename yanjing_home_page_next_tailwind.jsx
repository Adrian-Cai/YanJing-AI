...
const ROBOT_ASSET_SRC = "/assets/home/hero-robot.webp";
...
export const yanjingHomeSmokeTests = [
  { name: "top navigation has six items", pass: navItems.length === 6 },
  { name: "mock sidebar keeps personal center visible", pass: mockMenus.some((item) => item.label === "个人中心") },
  { name: "device mockup has four feature cards", pass: mockFeatures.length === 4 },
  { name: "stats bar has four metrics", pass: stats.length === 4 },
  { name: "advantages section has five cards", pass: advantages.length === 5 },
  { name: "icon fields are strings", pass: [...mockMenus, ...mockFeatures, ...stats, ...advantages].every((item) => typeof item.icon === "string") },
  { name: "robot asset uses a local public path", pass: ROBOT_ASSET_SRC.startsWith("/assets/") && !ROBOT_ASSET_SRC.startsWith("http") },
...
function VoiceBars({ compact = false, fill = false }: { compact?: boolean; fill?: boolean }) {
  const bars = fill
...
function DeviceMockup() {
  return (
    <div className="relative h-[470px] w-[780px] xl:h-[500px] xl:w-[840px]" data-testid="device-mockup">
      ...
      <div className="floating-card floating-device-card voice-floating-card absolute left-[26px] bottom-[54px] z-30 h-[72px] w-[164px] rounded-[16px] border border-white/75 bg-white/78 px-3 py-2.5 shadow-[0_18px_46px_rgba(68,93,175,0.16)] backdrop-blur-2xl" data-testid="voice-floating-card">
    ...
    <section className="bg-white px-10 py-14 xl:px-12" data-testid="advantages-section">
      <div className="mx-auto max-w-[1280px]">
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-2 xl:grid-cols-5">
          {advantages.map((item) => (
            <div key={item.title} className="group flex min-h-[132px] items-start gap-4 rounded-[22px] border border-slate-200 bg-white p-5 shadow-[0_14px_36px_rgba(15,23,42,0.06)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_22px_56px_rgba(37,99,235,0.14)]">
              <MiniIcon type={item.icon} color={item.color} />
...
export default function YanJingHomePage() {
  return (
    <main className="min-h-screen bg-white text-slate-950">
      ...
      <style>{`
        .device-shell {
          transform: perspective(1250px) rotateY(-13deg) rotateX(3deg) rotateZ(1.1deg);
        ...
        .device-shell::before {
          ...
          border-radius: 34px;
          background: linear-gradient(135deg, rgba(167, 191, 255, 0.26), rgba(84, 119, 255, 0.08));
          filter: blur(0.2px);
          z-index: -1;
        }
        .device-shell::after {
          ...
          height: 38px;
          border-radius: 999px;
          background: rgba(69, 100, 220, 0.22);
          filter: blur(22px);
          z-index: -2;
        }
        .floating-card {
        ...
        .floating-card:hover {
          ...
          box-shadow: 0 24px 64px rgba(50, 86, 200, 0.20);
        }
        .floating-device-card {
          transform: none;
        }
        .floating-device-card::before {
          content: "";
          position: absolute;
          inset: 1px;
          border-radius: inherit;
          background: linear-gradient(135deg, rgba(255, 255, 255, 0.58), rgba(239, 246, 255, 0.20));
          z-index: 0;
          pointer-events: none;
        }
        .floating-device-card::after {
          content: "";
          position: absolute;
          left: 10%;
          right: 10%;
          ...
          height: 14px;
          border-radius: 999px;
          background: rgba(69, 100, 220, 0.10);
          filter: blur(10px);
          z-index: -1;
          pointer-events: none;
        }
        @keyframes voiceBar {
          ...
          70% { transform: scaleY(0.72); opacity: 0.78; }
        }
        .animate-voice-bar {
